...
let init = false;
// TODO: Make it so it will somehow work with multiple sessions
global.JewelSession = async () => {
    if (init) return printer.error("You can only have one session at a time.");
    init = true;
    let cnfStr = await readFileAsync("./config.json", "utf8");
    const def = require("./config.json");
    if (cnfStr.err) {
        printer.error("Couldn't find a config file! Creating one...");
        fs.writeFileSync("./config.json", JSON.stringify(def, null, 4));
        cnfStr = def;
        printer.notice("Created a config file at ./config.json");
        printer.constructor.line = "";
        printer.notice("Press a key to continue...");
        printer.constructor.line = "\n";
        await ConsoleReader.readKey();
    } else cnfStr = JSON.parse(cnfStr.data);
    /*** @type {Object | {experimental,tmp,token,prefixCommands,slashCommands,errors,events,language,config}} */
    global.config = JSON.parse(JSON.stringify(cnfStr));
    if (config["no-crash"]) {
    ...
    if (config.experimental["update-check"].enabled) {
        ...
        try {
            ...
            printer.clear();
            const mapVer = str => str.split("-")[0].split(".").map((i, j) => i * (j + 1)).reduce((a, b) => a + b, 0);
            const currentVersion = mapVer(require("./package.json").version ?? "1.0.0");
            const cmd = `npm install discord-jewel@${latestTemplate.version}`;
            if (mapVer(latestTemplate.version) > currentVersion) {
                printer.warn(`New version available! To install run this command: ${cmd}`);
        ...
        for (let i = 0; i < commands.length; i++) {
            ...
            if (result.error) {
                ...
                if (config.errors["save-details"].enabled) {
                    ...
                    fs.writeFileSync(path.join(_dr, config.errors["save-details"].directory, file), save);
                }
            }
        }
    });
    ...
    if (config.language["auto-reload"].enabled) {
        auto(async () => {
            ...
            const dir = path.join(_dr, config.language.directory);
    ...
    if (config.prefixCommands["auto-reload"].enabled) {
        auto(async () => {
            ...
            const dir = path.join(_dr, config.prefixCommands.directory);
    ...
    if (config.slashCommands["auto-reload"].enabled) auto(async () => {
        ...
        const dir = path.join(_dr, config.slashCommands.directory);
        const files = fs.readdirSync(dir).filter(i => i.split(".").length === 2 && i.endsWith(".js"));
        let list = [];
    ...
    if (config.events["auto-reload"].enabled) auto(async () => {
        ...
        const dir = path.join(_dr, config.events.directory);
        const files = fs.readdirSync(dir).filter(i => i.split(".").length === 2 && i.endsWith(".js"));
        let list = [];
 ...
 */
module.exports = JewelSession;
JewelSession.MAIN_FILE = process.argv[1];